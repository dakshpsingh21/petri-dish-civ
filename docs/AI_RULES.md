...
- **Sim code (`sim.js`, `world.js`, `noise.js`, `agent.js`, `rules.js`, `minds.js`, `tribes.js`, `nature.js`, `commands.js`, `fx.js`, `chronicle.js`, `templates.js`, `stats.js`, `spatialHash.js`) never touches the DOM** and never calls `Math.random()`, `Date.now()` or `performance.now()`. Use `rng.js`.
